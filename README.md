...
### Stage 1: 0-200毫秒（或当|u_C| > 800V时停止）

#### 电路拓扑

在0-200毫秒阶段（或当电容电压绝对值超过800V时），SCR和K3所在回路尚未导通，短路电流通过R//C回路，再串Rg后通流。
...
- **方法**: Radau（适用于刚性问题）
- **时间范围**: 0 到 200 毫秒 (0 到 0.2 秒)，或当`|u_C| ≥ 800V`时提前停止
- **采样间隔**: 10 微秒 (0.00001 秒)
- **终止条件**: 当`|u_C| ≥ 800V`时触发事件，停止仿真
- **相对容差**: 1e-8
- **绝对容差**: 1e-10

**事件检测**:
- 使用`solve_ivp`的事件功能检测`|u_C| ≥ 800V`条件
- 事件函数返回`|u_C| - 800.0`，当从负值变为非负值时触发
- 事件触发后仿真立即停止，确保不超过安全阈值
...
### Stage 2: Stage 1结束时间-50毫秒

#### 电路拓扑

在Stage 1结束时（当`|u_C| ≥ 800V`或达到200ms时），电子旁路（整流桥）导通，电路拓扑发生变化。

**拓扑结构**: 电流源 → [L-整流桥 | R | C] → Rg → 地

**初始条件继承**:
- Stage 2的开始时间从Stage 1的CSV结果文件中读取（Stage 1的结束时间）
- Stage 2的初始`u_C`值从Stage 1的CSV结果文件中读取（Stage 1的最终`u_C`值）
- Stage 2的初始`i_L`值设为0（Stage 1没有电感）
...
- **求解器**: `scipy.integrate.solve_ivp`
- **方法**: Radau
- **时间范围**: 从Stage 1的结束时间到 50 毫秒（动态确定开始时间）
- **采样间隔**: 10 微秒 (0.00001 秒)
- **初始条件**: 从Stage 1的CSV结果文件中自动读取结束时间和`u_C`值
...
2. **初始条件**:
   ...
   - Stage 2自动从Stage 1的CSV结果文件中读取结束时间和`u_C`值作为初始条件
   - Stage 3自动从Stage 2的CSV结果文件中读取结束时间和`u_C`、`i_L`值作为初始条件
   - 各阶段的开始时间由上一阶段的结束时间决定，确保时间连续性

3. **采样间隔**:
   - Stage 1: 10微秒（平衡精度和效率）
   - Stage 2和Stage 3: 10微秒（平衡精度和效率）

4. **终止条件**:
   - Stage 1会在`|u_C| ≥ 800V`时提前停止，或运行到200ms
   - Stage 2和Stage 3运行到指定的结束时间（50ms和200ms）

5. **结果文件**:
   - CSV文件使用UTF-8-BOM编码，可在Excel中正确显示中文
   - 图片文件保存在`results/`目录，不会自动显示，仅保存
   - Stage 2和Stage 3会读取Stage 1和Stage 2的CSV文件来获取初始条件，确保运行顺序正确
