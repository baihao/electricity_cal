...
拓扑说明：
- 在50ms时，K3闭合
- K3闭合后，电路拓扑变为：K3 | (整流桥+L) | R | C（四个支路并联）
- L在整流桥内部，与SCR串联，i_L恒为正
- 在L回路中，晶闸管因为电流过小而关闭之后，将不再开启
- 当i_L < I_SCR_OFF_THRESHOLD时，(整流桥+L)支路断开，拓扑变为：K3 | R | C（三个支路并联）
- 大部分电流由K3接管
- 拓扑：电流源 → [K3 | (整流桥+L) | R | C] → Rg → 地
- 由于K3电阻很小，各并联支路电压接近0
...
1. 状态变量选择：
   ...
   - i_bypass：整流桥+L支路电流（可正可负，但i_L = |i_bypass|恒为正）

2. KCL方程（在电流源节点）：
   根据电路拓扑 K3 | (整流桥+L) | R | C（并联）：
   
   当L回路导通（i_L = |i_bypass| ≥ I_SCR_OFF_THRESHOLD）：
   i_s(t) = i_K3 + i_bypass + i_R + i_C
   其中：
   ...
   当L回路断开（i_L = |i_bypass| < I_SCR_OFF_THRESHOLD）：
   i_s(t) = i_K3 + i_R + i_C
   其中：
   - i_K3 = u_C / R_K3：K3回路电流
   - i_R = u_C / R：电阻电流
   - i_C = C · du_C/dt：电容电流
   - i_bypass：快速衰减到0（L回路断开，不再参与KCL）
   
   整流桥+L支路电流i_bypass的计算方式：
   * 当晶闸管关闭（i_L = |i_bypass| < I_SCR_OFF_THRESHOLD）：
     (整流桥+L)支路断开，i_bypass快速衰减到0
     di_bypass/dt = -i_bypass / τ_fast
     其中 τ_fast 为快速衰减时间常数
   * 当晶闸管还导通（i_L = |i_bypass| ≥ I_SCR_OFF_THRESHOLD）：
     KVL：(整流桥+L)支路：u_C = u_bridge + u_L
     其中 u_L = L · di_bypass/dt，u_bridge = sign(i_bypass)·2.38 + 0.000192·i_bypass
     因此：di_bypass/dt = (u_C - u_bridge) / L
     由于K3短接，u_C快速衰减到接近0，u_bridge也接近0
     所以i_bypass主要通过u_C的衰减而衰减

3. KVL方程（并联支路电压关系）：
   由于K3、R、C、(整流桥+L)并联，它们两端的电压相同：
   - u_K3 = u_C = u_R = u_bypass（当L回路导通时）
...
5. 状态方程：
   ...
   当L回路导通（i_L = |i_bypass| ≥ I_SCR_OFF_THRESHOLD）：
   - KCL：i_s = i_K3 + i_bypass + i_R + i_C
     其中：i_K3 = u_C / R_K3, i_R = u_C / R, i_C = C · du_C/dt
     因此：i_s = u_C/R_K3 + i_bypass + u_C/R + C·du_C/dt
     整理得到：du_C/dt = (i_s - i_bypass - u_C/R - u_C/R_K3) / C
   ...
   - i_bypass衰减：di_bypass/dt = -i_bypass / τ_fast（快速衰减到0）
...
def stage3_ode(t: float, y: np.ndarray) -> np.ndarray:
    ...
    # 检查晶闸管是否关闭（i_L < 阈值）
    scr_off = i_l < I_SCR_OFF_THRESHOLD
    ...
    if scr_off:
        ...
        # i_bypass衰减：L回路已断开，i_bypass应该快速衰减到0
        # 为了避免数值问题，使用一个很小的固定衰减时间常数
        if abs(i_bypass) < 1e-6:
            # i_bypass已经非常小，直接设置为0，避免数值问题
            di_bypass_dt = 0.0
        else:
            # 使用一个很小的固定衰减时间常数（例如1微秒），让i_bypass快速衰减
            tau_fast = 1e-6  # 1微秒的衰减时间常数
            di_bypass_dt = -i_bypass / tau_fast
    else:
        # L回路导通，拓扑：K3 | (整流桥+L) | R | C
        # KCL：i_s = i_K3 + i_bypass + i_R + i_C
        # 其中：i_C = C · du_C/dt
        # 因此：i_s = u_C/R_K3 + i_bypass + u_C/R + C·du_C/dt
        # 整理得到：du_C/dt = (i_s - i_bypass - u_C/R - u_C/R_K3) / C
        du_c_dt = (i_s - i_bypass - i_r - i_k3) / C
...
def simulate_stage3(u0: float, i_bypass0: float = 0.0, t_start: float = T_START, 
    ...
    参数：
        ...
        i_bypass0: 初始整流桥+L支路电流（A），从Stage 2的最终值继承
    ...
    # 验证KCL：i_s = i_K3 + i_bypass + i_R + i_C（当L回路导通时）
    ...
    # 计算各元件电压
    # 由于K3、R、C、(整流桥+L)并联，它们两端的电压都是u_C
...
def print_summary(t: np.ndarray, u_c: np.ndarray, i_bypass: np.ndarray, results: dict, u0: float) -> None:
    ...
    print(f"  i_bypass(50ms) = {i_bypass[0]:.6f} A")
    print(f"  i_L(50ms) = {results['i_l'][0]:.6f} A (恒为正)")
    ...
    print(f"\n验证（KCL）:")
    if results['i_l'][-1] < I_SCR_OFF_THRESHOLD:
        # L回路断开
        ...
        # L回路导通
        print(f"  i_s = i_K3 + i_bypass + i_R + i_C (L回路导通):")
        kcl_sum = results['i_k3'][-1] + i_bypass[-1] + results['i_r'][-1] + results['i_c'][-1]
        print(f"    {results['i_s'][-1]:.6f} ≈ {kcl_sum:.6f}")
        print(f"  误差: {abs(results['i_s'][-1] - kcl_sum):.6e} A")
    print(f"\n注意：")
    print(f"  - 拓扑：K3 | (整流桥+L) | R | C（并联）")
...
if __name__ == "__main__":
    ...
    try:
        u_c_stage2_final, i_bypass_stage2_final = get_stage2_final_values(stage2_u0_case1, results_dir)
        print(f"从Stage 2 (u0={stage2_u0_case1}V) 继承:")
        print(f"  u_C(50ms) = {u_c_stage2_final:.6f} V")
        print(f"  i_bypass(50ms) = {i_bypass_stage2_final:.6f} A")
        print(f"  i_L(50ms) = {abs(i_bypass_stage2_final):.6f} A (恒为正)")
        u0_case1 = u_c_stage2_final
        i_bypass0_case1 = i_bypass_stage2_final
    except FileNotFoundError:
        print(f"警告: 未找到Stage 2的结果文件 (u0={stage2_u0_case1}V)，使用默认值")
        u0_case1 = U0_CASE1
        i_bypass0_case1 = 0.0
    
    t1, u_c1, i_bypass1, results1 = simulate_stage3(u0_case1, i_bypass0_case1)
    ...
    try:
        u_c_stage2_final, i_bypass_stage2_final = get_stage2_final_values(stage2_u0_case2, results_dir)
        print(f"\n从Stage 2 (u0={stage2_u0_case2}V) 继承:")
        print(f"  u_C(50ms) = {u_c_stage2_final:.6f} V")
        print(f"  i_bypass(50ms) = {i_bypass_stage2_final:.6f} A")
        print(f"  i_L(50ms) = {abs(i_bypass_stage2_final):.6f} A (恒为正)")
        u0_case2 = u_c_stage2_final
        i_bypass0_case2 = i_bypass_stage2_final
    except FileNotFoundError:
        print(f"警告: 未找到Stage 2的结果文件 (u0={stage2_u0_case2}V)，使用默认值")
        u0_case2 = U0_CASE2
        i_bypass0_case2 = 0.0
    
    t2, u_c2, i_bypass2, results2 = simulate_stage3(u0_case2, i_bypass0_case2)
